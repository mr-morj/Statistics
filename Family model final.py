...
def subplots_graph(n_a, n_he, n_l, n_de, n_re, acc, ind):
    accomodation = 'without accumulation'
    indent = '(expected value)'
    if (acc==1):
        accomodation = 'with accumulation'
    ...
    plt.savefig(f"barplot {accomodation} {indent}.png")
    plt.show()
...
def MC_simulation(times):
    ...
    subplots_graph(mean_a_days, mean_he_days, mean_l_days, mean_de_days, mean_re_days, 0, 0)
    subplots_graph(mean_a_acc_days, mean_he_days, mean_l_acc_days, mean_de_acc_days, mean_re_days, 1, 0)
    ...
    subplots_graph(std_a_days, std_he_days, std_l_days, std_de_days, std_re_days, 0, 1)
    ...
    eps = float(input('Enter the (1-p) of confidence interval: '))
